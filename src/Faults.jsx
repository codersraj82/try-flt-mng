...
function Faults() {
  ...
  const formatDateForDisplay = (dateStr) => {
    ...
    return isNaN(date) ? dateStr : format(date, "dd/MM/yyyy HH:mm");
  };
  ...
  useEffect(() => {
    ...
    if (!startRaw) {
      setFormData((prev) => ({ ...prev, "Fault durration (Hrs)": "" }));
      return;
    }
  ...
  return (
    <div style={{ marginTop: "40px", padding: "0 20px" }}>
      ...
      {/* Faults list rendering unchanged */}
      {/* You already have the rest of the component correct */}
    </div>
  );
}
